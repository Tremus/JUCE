...
public:
    ...
    */
    ...
    /** This method must be implemented to draw a row of the list.
        Note that the rowNumber value may be greater than the number of rows in
       your list, so be careful that you don't assume it's less than
       getNumRows().
    */
    virtual void paintListBoxItem(
        int       rowNumber,
        Graphics& g,
        int       width,
        int       height,
    ...
    /** This is used to create or update a custom component to go in a row of
       the list.

        Any row may contain a custom component, or can just be drawn with the
       paintListBoxItem() method and handle mouse clicks with
       listBoxItemClicked().

        This method will be called whenever a custom component might need to be
       updated - e.g. when the list is changed, or ListBox::updateContent() is
       called.

        If you don't need a custom component for the specified row, then return
       ...
       null, then this method must create a suitable new component and return
       it.

        If the existingComponentToUpdate is non-null, it will be a pointer to a
       component previously created by this method. In this case, the method
       must either update it to make sure it's correctly representing the given
       row (which may be different from the one that the component was created
       for), or it can delete this component and return a new one.

        The component that your method returns will be deleted by the ListBox
       when it is no longer needed.

        Bear in mind that if you put a custom component inside the row but still
       want the listbox to automatically handle clicking, selection, etc, then
       you'll need to make sure your custom component doesn't intercept all the
       mouse events that land on it, e.g by using
       Component::setInterceptsMouseClicks().
    */
    virtual Component* refreshComponentForRow(
        int        rowNumber,
        bool       isRowSelected,
    ...
    */
    ...
    /** This can be overridden to react to the user clicking on a part of the
    ...
    */
    ...
    /** Override this to be informed when rows are selected or deselected.
        ...
        This will be called whenever a row is selected or deselected. If a range
       of rows is selected all at once, this will just be called once for that
       event.
    ...
    */
    ...
    /** Override this to be informed when the delete key is pressed.
        ...
        @param lastRowSelected   the last row that had been selected when they
       pressed the key - if there are multiple selections, this might not be
                                 very useful
    */
    ...
    */
    ...
    /** Override this to be informed when the list is scrolled.
        ...
        This might be caused by the user moving the scrollbar, or by
       programmatic changes to the list position.
    */
    virtual void listWasScrolled();

    /** To allow rows from your list to be dragged-and-dropped, implement this
       method.

        If this returns a non-null variant then when the user drags a row, the
       listbox will try to find a DragAndDropContainer in its parent hierarchy,
       and will use it to trigger a drag-and-drop operation, using this string
       as the source description, with the listbox itself as the source
       component.
    ...
    */
    ...
    /** Called when starting a drag operation on a list row to determine whether
       the item may be dragged to other windows. Returns true by default.
    */
...
/**
    ...
    either paint each row of the list and respond to events via callbacks, or
   for more specialised tasks, it can supply a custom component to fill each
   row.
...
public:
    ...
    /** Creates a ListBox.
        ...
        The model pointer passed-in can be null, in which case you can set it
       later with setModel().
    ...
    */
    ...
    /** If enabled, this makes the listbox flip the selection status of
        ...
        If not enabled, you can still get the same row-flipping behaviour by
       holding down CMD or CTRL when clicking.
    */
    void setClickingTogglesRowSelection(bool flipRowSelection) noexcept;

    /** Sets whether a row should be selected when the mouse is pressed or
       released. By default this is true, but you may want to turn it off.
    */
    void setRowSelectedOnMouseDown(bool isSelectedOnMouseDown) noexcept;

    /** Gets whether a row should be selected when the mouse is pressed or
       released. By default this is true, but you may want to turn it off.
    */
    bool getRowSelectedOnMouseDown() const { return selectOnMouseDown; }

    /** Makes the list react to mouse moves by selecting the row that the mouse
       if over.

        This function is here primarily for the ComboBox class to use, but might
    ...
    */
    ...
    /** Selects a row.
        ...
        @param dontScrollToShowThisRow  if true, the list's position won't
       change; if false and the selected row is off-screen, it'll scroll to make
                                        sure that row is on-screen
        @param deselectOthersFirst      if true and there are multiple
       selections, these will first be deselected before this item is selected
        @see isRowSelected, selectRowsBasedOnModifierKeys, flipRowSelection,
       deselectRow, deselectAllRows, selectRangeOfRows
    */
    void selectRow(
        int  rowNumber,
    ...
    /** Selects a set of rows.
       ...
       (inclusive)
        ...
        @param dontScrollToShowThisRange      if true, the list's position won't
       change; if false and the selected range is off-screen, it'll scroll to
       make sure that the range of rows is on-screen
    */
    ...
    */
    ...
    /** Sets the rows that should be selected, based on an explicit set of
       ranges.

        If sendNotificationEventToModel is true, the
       ListBoxModel::selectedRowsChanged() method will be called. If it's false,
    ...
    */
    ...
    /** Returns the row number of a selected row.
        ...
        This will return the row number of the Nth selected row. The row numbers
       returned will be sorted in order from low to high.

        @param index    the index of the selected row to return, (from 0 to
       getNumSelectedRows() - 1)
        @returns        the row number, or -1 if the index was out of range or
       if there aren't any rows selected
    ...
    */
    ...
    /** Returns the last row that the user selected.
        ...
        This isn't the same as the highest row number that is currently selected
       - if the user had multiply-selected rows 10, 5 and then 6 in that order,
       this would return 6.
    ...
    */
    ...
    /** Multiply-selects rows based on the modifier keys.
        ...
        @see selectRow
    */
    void selectRowsBasedOnModifierKeys(
    ...
    /** Scrolls the list to a particular position.
        ...
        The proportion is between 0 and 1.0, so 0 scrolls to the top of the
       list, 1.0 scrolls to the bottom.
    ...
    */
    ...
    /** Returns the current vertical position as a proportion of the total.
        ...
        This can be used in conjunction with setVerticalPosition() to save and
       restore the list's position. It returns a value in the range 0 to 1.
    ...
    */
    ...
    /** Finds a row index that would be the most suitable place to insert a new
        ...
        This is useful when the user is e.g. dragging and dropping onto the
       listbox, because it lets you easily choose the best position to insert
       the item that they drop, based on where they drop it.

        If the position is out of range, this will return -1. If the position is
        beyond the end of the list, it will return getNumRows() to indicate the
       end of the list.
    ...
    */
    ...
    /** Returns the position of one of the rows, relative to the top-left of
        ...
        This may be off-screen, and the range of the row number that is
       passed-in is not checked to see if it's a valid row.
    */
    Rectangle<int> getRowPosition(
        int  rowNumber,
        bool relativeToComponentTopLeft) const noexcept;

    /** Finds the row component for a given row in the list.

        The component returned will have been created using
       ListBoxModel::refreshComponentForRow().

        If the component for this row is off-screen or if the row is
       out-of-range, this will return nullptr.
    ...
    */
    ...
    /** Returns the width of a row (which may be less than the width of this
       component if there's a scrollbar).
    */
    ...
    */
    ...
    /** Returns the number of rows actually visible.
        ...
        This is the number of whole rows which will fit on-screen, so the value
       might be more than the actual number of rows in the list.
    */
    ...
    enum ColourIds
    {
        backgroundColourId =
            0x1002800, /**< The background colour to fill the list with.
                            Make this transparent if you don't want the
                          background to be filled. */
        outlineColourId = 0x1002810, /**< An optional colour to use to draw a
                                        border around the list. Make this
                                        transparent to not have an outline. */
        textColourId = 0x1002820 /**< The preferred colour to use for drawing
                                    text in the listbox. */
    };

    /** Sets the thickness of a border that will be drawn around the box.

        To set the colour of the outline, use @code setColour
       (ListBox::outlineColourId, colourXYZ); @endcode
        @see outlineColourId
    */
    ...
    */
    ...
    /** Sets a component that the list should use as a header.
        ...
        This will position the given component at the top of the list,
       maintaining the height of the component passed-in, but rescaling it
       horizontally to match the width of the items in the listbox.
    ...
    */
    ...
    /** Changes the width of the rows in the list.
        ...
        This can be used to make the list's row components wider than the list
       itself - the width of the rows will be either the width of the list or
       this value, whichever is greater, and if the rows become wider than the
       list, a horizontal scrollbar will appear.
    ...
    */
    ...
    /** Returns the space currently available for the row items, taking into
       account borders, scrollbars, etc.
    */
    int getVisibleContentWidth() const noexcept;

    /** Repaints one of the rows.

        This does not invoke updateContent(), it just invokes a straightforward
       repaint for the area covered by this row.
    */
    void repaintRow(int rowNumber) noexcept;

    /** This fairly obscure method creates an image that shows the row
       components specified in rows (for example, these could be the currently
       selected row components).
        ...
        Note that it will make the row components temporarily invisible, so if
       you're using custom components this could affect them if they're
       sensitive to that sort of thing.

        @see Component::createComponentSnapshot
    */
    virtual ScaledImage
    createSnapshotOfRows(const SparseSet<int>& rows, int& x, int& y);
    ...
    */
    ...
    //==============================================================================
#ifndef DOXYGEN
    [[deprecated("This method's bool parameter has changed: see the new method "
                 "signature.")]] void
    setSelectedRows(const SparseSet<int>&, bool);
#endif

    [[deprecated(
        "The name of this function is ambiguous if derived classes supply "
        "their own models, use getListBoxModel instead")]] ListBoxModel*
...
private:
    ...
    bool multipleSelection = false, alwaysFlipSelection = false,
         hasDoneInitialUpdate = false, selectOnMouseDown = true;
